...
let clsOld: ContinuationLocalStorage<Number> =
    new ContinuationLocalStorage<Number>();
...
describe('test continuation but enable hooks right before each test:', () => {
  ...
  it('calling process.nextTick should preserve continuation local storage',
     async(done) => {
       ...
       expect(cls.setRootContext(startValue))
           .toBe(startValue, `start value (${startValue}) not set`);
       ...
       process.nextTick(() => {
         ...
         expect(cls.setContext(outerValue))
             .toBe(outerValue, `outer value (${outerValue}) not set`);
         ...
         process.nextTick(() => {
           ...
           expect(innerId).toBeGreaterThan(
               outerId,
               'inner id (${innerid}) is not greater outer id (${outerid})');
           ...
           expect(cls.setContext(innerValue))
               .toBe(innerValue, `inner value (${innerValue}) not set`);
  ...
  it('calling setImmediate should preserve continuation local storage', async(
                                                                            done) => {
    cls.enable();
  ...
  it('calling setTimeout should preserve continuation local storage', async(
                                                                          done) => {
    cls.enable();
    debugId('START BEGIN');
    const startId = cls.currId;
  ...
  it('calling setInterval should preserve continuation local storage', async(
                                                                           done) => {
    cls.enable();
  ...
  it('chained promises should preserve continuation local storage', (done) => {
    ...
    return new Promise<number>((resolve, reject) => {
             ...
             expect(outerId).toBe(
                 startId,
                 `my id (${outerId}) is not the expected start id (${startId})`);
             expect(cls.getContext())
                 .toBe(
                     startValue,
                     `outer value is not the expected start value (${startValue})`);
              ...
              .then((val2) => {
                ...
                return new Promise<number>((resolve, reject) => {
                  ...
                  expect(innermostId)
                      .toBeGreaterThan(
                          innerId,
                          'innermost id (${innermostid}) is not greater inner id (${innerid})');
