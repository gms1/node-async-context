...
var OperationTSCompile = (function() {
  ...
  OperationTSCompile.prototype.files = function() {
    const {fileNames, errors} = ts.parseJsonConfigFileContent(this.tsConfig, ts.sys, this.tsConfigDirname);
  ...
  OperationTSCompile.prototype.watch = function() {
    ...
    var globs = [];
    this.tsConfig.include.forEach((glob) => { globs.push(path.join(this.tsConfigDirname, glob)); });
    if (this.tsConfig.exclude && this.tsConfig.exclude.length) {
      this.tsConfig.exclude.forEach((glob) => { globs.push('!' + path.join(this.tsConfigDirname, glob)); });
    }
    return globs;
  };
